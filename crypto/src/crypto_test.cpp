...
TEST(Signature, CanVerifySignatures)
{
    ...
    for (const auto& tc : test_cases) {
        ...
        ASSIGN_OR_FAIL(bool valid, mw::verifySignature(
        ...
        if (!signature.empty()) {
            signature[0] ^= 0xFF;
            ASSIGN_OR_FAIL(bool invalid, mw::verifySignature(
                tc.algo, pub_key, signature, data));
...
TEST(Signature, CanVerifyHMAC)
{
    ...
    ASSIGN_OR_FAIL(bool valid, mw::verifySignature(algo, key, signature, data));
    EXPECT_TRUE(valid);

    signature[0] ^= 0xFF;
    ASSIGN_OR_FAIL(bool invalid, mw::verifySignature(algo, key, signature, data));
...
TEST(Signature, CanSignAndVerify)
{
    ...
    for (const auto& tc : test_cases)
    {
        ...
        ASSIGN_OR_FAIL(auto signature, mw::sign(tc.algo, priv_key_pem, data));

        ASSIGN_OR_FAIL(bool valid,
                       mw::verifySignature(tc.algo, pub_key_pem, signature, data));
        EXPECT_TRUE(valid)
    ...
    // HMAC test
    {
        std::string key(32, 'k');
        mw::SignatureAlgorithm algo = mw::SignatureAlgorithm::HMAC_SHA256;
        ASSIGN_OR_FAIL(auto signature, mw::sign(algo, key, data));
        ASSIGN_OR_FAIL(bool valid, mw::verifySignature(algo, key, signature, data));
        EXPECT_TRUE(valid);
    }
}
